...
| `limit-boundary`          | `concerns`; an introduced `SHOULD FIX`: equality is one bounded input condition, so the defect does not become a `BLOCKER` merely because the change exists to handle it. Following the faulty plan does not excuse contradicting the issue.      |
...
| `unclassified-recheck`    | `incomplete`; `earlier-unknown` is `undetermined` because the relevant source and runtime evidence remain unavailable. Its earlier null severity may still conceal blocking harm, so the result records a limitation instead of clearing it.      |
